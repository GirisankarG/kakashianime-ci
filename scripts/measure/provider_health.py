...
def report(dead: list[str], score: dict, n: int) -> None:
    ...
    if not dsn or not dead:
        return
    try:
        import sentry_sdk
    except ImportError:
        print("  SENTRY_DSN set but sentry_sdk is not installed; skipping")
        return
    sentry_sdk.init(dsn=dsn, traces_sample_rate=0)
    sentry_sdk.set_context("providers", {k: f"{v}/{n}" for k, v in score.items()})
    sentry_sdk.capture_message(
        f"KakashiAnime: {len(dead)} provider(s) play nothing: {', '.join(dead)}",
        level="error")
    sentry_sdk.flush(5)
    print(f"  reported {len(dead)} dead provider(s) to Sentry")
...
def providers() -> list[tuple[str, str, str]]:
    ...
    if raw:
        return [(p["name"], p["tmpl"], p["key"]) for p in json.loads(raw)]
    sys.path.insert(0, str(ROOT))
    from scrapers.universal import UNIVERSAL      # noqa: E402
    return [(p["name"], p["tmpl"], p["key"]) for p in UNIVERSAL]
